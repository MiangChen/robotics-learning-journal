# 集群任务规划（CN）
- 主要调研关于集群机器人的任务规划算法，在论文顺序上，将按照该算法在该领域的发展阶段中的位置来排序
...
- [算法论文](#算法论文)
  - [描述语言](#描述语言)
      - [PDDL - The Planning Domain Definition Language](#pddl---the-planning-domain-definition-language)
      - [HTN - The Nonlinear Nature of Plans](#htn---the-nonlinear-nature-of-plans)
  - [基于市场机制的博弈方法](#基于市场机制的博弈方法)
  - [基于显式优化的规划方法](#基于显式优化的规划方法)
      - [Robust Task Scheduling for Heterogeneous Robot Teams under Capability Uncertainty](#robust-task-scheduling-for-heterogeneous-robot-teams-under-capability-uncertainty)
  ...
  - [基于学习驱动的决策方法](#基于学习驱动的决策方法)
      - [An LLM-based Framework for Human-Swarm Teaming Cognition in Disaster Search and Rescue](#an-llm-based-framework-for-human-swarm-teaming-cognition-in-disaster-search-and-rescue)
...
- [混合架构 基于显式优化 + 基于学习驱动的决策方法](#混合架构-基于显式优化-基于学习驱动的决策方法)
      - [LaMMA-P: A Framework for Language-Model-based Multi-Agent Planning with PDDL](#lamma-p-a-framework-for-language-model-based-multi-agent-planning-with-pddl)
- [拓展](#拓展)
...
- 主旨思想
  ...
  <img src="asset/集群任务规划.asset/截屏2025-11-04 10.26.17.png" style="zoom: 100%;" />

  - Fig3根据任务分配和存在的调度关系，明确了Dependency Types，核心问题在讨论“执行这个任务会如何影响其他任务的执行？”。下图中的线不仅仅表示时间依赖的先后顺序，也表示资源消耗的依赖关系。

  <img src="asset/集群任务规划.asset/截屏2025-11-04 10.34.39.png" style="zoom: 100%;" />
...
- https://www.researchgate.net/profile/Craig-Knoblock/publication/2278933_PDDL_-_The_Planning_Domain_Definition_Language/links/0912f50c0c99385e19000000/PDDL-The-Planning-Domain-Definition-Language.pdf
- 1998.10
- Reference: https://planning.wiki/ref/pddl

PDDL 最初是在 **1998 年**第一届国际规划竞赛（International Planning Competition, IPC）中由 **Drew McDermott 及其同事**提出的，最初是被作为一个技术报告发表。

PDDL出现之前，人工智能规划领域的各个研究团队和规划器系统都有自己独特的输入格式，他们缺乏一个标准的\统一的语言，各自为战，这有一个很明显的问题:

- 难以公平比较，无法对不同的规划器进行经验性的性能评估
- 难以共享和复用: 研究人员很难共享问题集和算法，整个领域的发展缓慢



为此，该论文参考了当时的ADL\SIPE2\Prodigy4.0等等，吸取各家优点推出了PDDL.

其核心思想是领域(Domain)和问题(Problem):

- **领域 (Domain):** 用于描述一个世界的“物理规则”（physics）。这部分定义了通用的概念，比如有哪些**类型 (types)** 的对象，有哪些可以执行的**动作 (actions)**，以及执行这些动作所需的**前提条件 (precondition)** 和会产生的**效果 (effect)**。这部分是通用的，可以被多个具体问题复用。
- **问题 (Problem):** 用于描述一个需要解决的具体场景。这部分定义了当前场景下有哪些具体的**对象 (objects)**，系统的**初始状态 (:init)** 是什么，以及期望达成的**目标状态 (:goal)** 是什么。



###### 例子 失落群岛的寻宝任务

让我们使用之前的寻宝任务来讲解如何用PDDL来描述问题，我们使用最简单的无依赖(ND)案例。
...
根据PDDL的要求，我们需要先创建一个领域(Domain)文件，它包含类型/动作/前提条件/效果，然后还需要创建一个问题(Problem)来描述具体的场景。



让我们先试着分析下这个寻宝任务 

在这个任务中，我们有3中对象，机器人(robot)，宝藏(treasure)，位置(location)

机器人的动作包括: 移动(move)，携带宝藏(carrying)，放宝藏(put down) 3个基础动作
其中，移动技能需要输入机器人的当前位置和目标位置，然后调用已经有的导航算法(比如Astar)来实现，在这里我们只关心上层的移动技能的参数即可
取宝藏的要求是 机器人需要在宝藏的位置，并且宝藏也还在这个位置(没有被别的机器人拿走)，然后机器人的背包里是空的，才能执行这个技能
放宝藏这个技能使用的要求是机器人必须在大本营，并且机器人携带着宝藏



我们可以用3个最简单的事情(PDDL中称之为 谓词)来描述上述的状态: 机器人在哪里(robot-at)，宝藏在哪里(treasure-at)，机器人的背包是不是空的(package-empty)，机器人携带了什么宝藏(carrying)

 

根据PDDL的**领域 (Domain)**文件，我们将上述的分析再梳理一下:

```pddl
# lost-island-domain.pddl
(define (domain lost-island)
  ;; 在PDDL中 两个分号;表示注释
  ;; :strips 是基础需求，:typing 允许我们定义对象的类型
  (:requirements :strips :typing)

  ;; 3种对象
  (:types
    robot treasure location
  )

  ;; 用4个最简单的事实来描述世界的状态
  (:predicates
    (robot-at ?r - robot ?l - location)         ; 机器人在哪里
    (treasure-at ?t - treasure ?l - location)   ; 宝藏在哪里
    (package-empty ?r - robot)                  ; 机器人的背包是否空闲
    (carrying ?r - robot ?t - treasure)         ; 机器人正在携带什么宝藏
  )

  ;; 机器人的动作包括: 移动(move), 取宝藏(pick-up), 放宝藏(put-down) 3个基础动作
  ...
  ;; 动作二：取宝藏 (pick-up)
  (:action pick-up
    :parameters (?r - robot ?t - treasure ?l - location)
    :precondition (and
      (robot-at ?r ?l)
      (treasure-at ?t ?l)
      (package-empty ?r)
  ...
  ;; 动作三：放宝藏 (put-down)
  (:action put-down
    :parameters (?r - robot ?t - treasure)
    :precondition (and
      (carrying ?r ?t)
      (at ?r base) ; <--- 核心约束：这个动作只能在大本营(base)发生！
    )
    :effect (and
      (package-empty ?r)
      (treasure-at ?t base)
...
​	通过上述过程，我们将自己用**自然语言描述的任务，转化到了规范化的PDDL domain文件**. 现在，不仅我们和别人能对失落群岛这个世界的描述基本清晰了，计算机也能理解了。 PDDL domain文件主要就是为了让计算机能规范化理解和计算。



​	下面，回到无依赖(ND)的任务这件事情上，我们希望位于基地的两个机器人能够把4个宝藏都带到基地。 这么简单一句话，在转化为problem文件前，还需要稍微拆分一下:

- 有两个机器人，分别叫做R1和R2
- 机器人位于基地的位置
- 机器人刚开始的背包是空的
- 基地的位置记作base
- 4个宝藏分别叫做 coin, goblet, skull, compass
- 宝藏分别位于4个位置, 分别记作 loc-a, loc-b, loc-c, loc-d



​	我们把信息量巨大的一句话拆解了一下，居然得到了5句，可以看到自然语言是存在相当丰富的信息的。

​	按照PDDL的 **问题 (Problem)** 文件的要求，可以将上面5句话用规范化的语言写出来:

```
# lost-island-problem.pddl
(define (problem find-4-treasures)
  (:domain lost-island) ; 

  ;; 定义这次任务中具体有哪些对象
  (:objects
    r1 r2 - robot
    base loc-a loc-b loc-c loc-d - location
    coin goblet skull compass - treasure
  )
  ...
  (:init
    ...
    ;; 宝藏的初始位置
    (treasure-at coin loc-a)
    (treasure-at goblet loc-b)
    (treasure-at skull loc-c)
    (treasure-at compass loc-d)
  )

  ;; 定义任务的最终目标
  (:goal (and
      (treasure-at coin base)
      (treasure-at goblet base)
      (treasure-at skull base)
      (treasure-at compass base)
    )
  )
)
```
...
​	有了上面 PDDL 的 **领域 (Domain)** 和 **问题 (Problem)** 文件后，我们就成功地将一个复杂的寻宝任务，用一种标准、严谨且机器可以理解的方式进行了形式化描述。 

​	这可以用我们后面提到的基于市场机制的博弈方法/显式优化的规划方法以及基于学习的方法等等算法来解决了。 这让我们能够专注于开发更高效和智能的算法，而不是每次都重复造轮子，研究如何描述问题。

***

##### HTN - The Nonlinear Nature of Plans

- https://www.ijcai.org/Proceedings/75/Papers/028.pdf
- 1975
- Proceedings of the 4th International Joint Conference on Artificial Intelligence (IJCAI)

在20世界70年代，就已经有了经典的规划器，比如以STRIPS为代表的先河，但那个时候他们是用目标为导向，而不是以过程为导向，存在一些低效的问题。



本文举了一个例子，要把3个方块从下图中左边的状态，转移到右图的状态。

![image-20251117133931090](./asset/集群任务规划.asset/image-20251117133931090.png)

这个问题可以分为两个子目标，1是要让A在B上面，2是B在C上面。 也就是G1(on A B)和G2(on B C)

早期的规划器会尝试一系列的动作来完成这个目标，但是他们解决子目标的时候是各自孤立的，并且存在干扰的可能性

比如，如果规划器决定先完成  G1(on A B)，它的过程如下所示: 

1. **实现G1 (on A B):**
   - 规划器检查：要把 A 放到 B 上，A 和 B 的顶部都必须是空的 (clear)。
   - 现状：B 是 clear 的，但 A 不是（因为 C 在 A 上）。
   - **动作1:** 必须先把 C 从 A 上拿走。拿到哪里去呢？放到桌子上最省事。所以执行 unstack(C, A) 然后 putdown(C)。
   - **当前世界状态:** A, B, C 三个积木都各自在桌子上。
   - **动作2:** 现在 A 和 B 都 clear 了，可以把 A 放到 B 上了。执行 pickup(A) 然后 stack(A, B)。
2. **完成 G1**
   - 此时，世界状态是：A 在 B 上，C 自己在桌子上。(on A B), (ontable C)。
3. **现在，开始完成G2 (on B C):**
   - 规划器检查：要把 B 放到 C 上，B 和 C 的顶部都必须是 clear 的。
   - 现状：C 是 clear 的，但 B **不是**（因为在上一步把 A 堆在了 B 上面，每次只能移动一个物块，无法叠着挪动）。
4. **冲突出现**
   - 为了实现 G2，规划器现在必须把 A 从 B 上面拿走，以此来让 B 变 clear。
   - 但这恰恰破坏了刚才完成的第一个目标 G1 (on A B)

这就是“**子目标间的相互干扰**”。为了实现第二个目标，不得不撤销第一个目标的成果。



我们可以继续用寻宝的例子来举例，过去规划器的缺陷

使用PDDL描述的规划器在面对我们的寻宝任务时，就像一个只有地图和指南针、但**毫无经验**的新手探险家。它知道目标是让所有宝藏都出现在大本营，也知道自己会 move, pickup, put-down 这几个动作，但它缺乏**策略和常识**。

在巨大的状态空间里，它可能会进行这样的“盲目搜索”：

1. 机器人 R1 从大本-营出发，成功移动到 A 点，拿起了“古代钱币”。
2. 此刻，它没有返回大本营的“常识”。它的内部状态显示“我拿着钱币”，目标是“让所有宝藏到家”。它可能会计算出 B 点的“宝石高脚杯”离得也不远，于是它做出了一个**不合逻辑**的决定：**直接带着“古代钱币”前往 B 点**。
3. 当 R1 到达 B 点时，它发现无法拿起“宝石高脚杯”，因为它的背包不是空的。
4. 为了解决这个新问题，它唯一的选择是：先返回大本营放下“古代钱币”，**然后再重新出发，一路跋涉回到 B 点**去拿高脚杯。

这使得最终生成的计划变得极其冗长和低效（例如：Base → A → B → Base → B → Base），并且规划过程本身也耗费了巨大的计算资源来排除这些路径。



**HTN 的“专家级寻宝策略”**

为了解决这种问题，作者观察到人类解决复杂问题时，不会从 0 开始思考每一个原子步骤，而是进行**层次化思考**。

比如一个**经验丰富的探险队长**会这样规划：

**第一层思考 (战略层面):** “我们的总任务是‘取回所有宝藏’。根据规则，我们必须一次运送一件，所以我们将整个探险分解为四次独立的往返任务。”

- **子任务 1**: “执行一次完整的 A 点寻宝往返任务。”
- **子任务 2**: “执行一次完整的 B 点寻宝往返任务。”
- **子任务 3**: “执行一次完整的 C 点寻宝往返任务。”
- **子任务 4**: “执行一次完整的 D 点寻宝往返任务。”

**第二层思考 (战术层面):** “一次‘完整的寻宝往返任务’具体应该这么做”

- **步骤 1**: “从大本营出发，移动到宝藏所在地。”
- **步骤 2**: “拾取宝藏。”
- **步骤 3**: “从宝藏所在地，返回大本营。”
- **步骤 4**: “在大本营卸下宝藏。”

这种操作类似“标准作业流程 (SOP)。 HTN 中称之为**方法 (Methods)** ，它将高层的、抽象的任务（如“取回宝藏 A”）分解为一系列逻辑清晰、顺序正确的具体步骤。



**完整的HTN的范式包括以下几个部分** 

1. **任务 (Tasks)**: 这是 HTN 的基本单元，分为两种：
   - **原始任务 (Primitive Tasks)**: 相当于 PDDL 中的“动作”，是机器人可以直接执行的原子操作，例如 move, pick-up。
   - **复合任务 (Compound Tasks)**: 抽象的高层任务，本身不能直接执行，必须被分解，例如 fetch-a-treasure。
2. **方法 (Methods)**: 这是 HTN 的灵魂，一个方法描述了**一种**将复合任务分解为一系列子任务（可以是原始的，也可以是更低层次的复合任务）的方式。一个复合任务可以有多种分解方法。例如，“去机场”这个复合任务，可以有“开车去”、“坐地铁去”等多种方法。
3. **领域 (Domain)**: 由所有可能的原始任务、复合任务和方法共同组成。
4. **问题 (Problem)**: 由两部分组成：
   - **初始状态 (Initial State)**: 和 PDDL 一样，描述世界最初的样子。
   - **初始任务网络 (Initial Task Network)**: 需要完成的顶层任务列表，例如 (and (fetch coin) (fetch goblet) ...)。



###### 例子 失落群岛的寻宝任务

我们继续用失落群岛任务来完整地演示一下如何使用HTN，并得到2个对应的文件，可以分为4步



**第一步, 定义原始任务(Primitive Task)**

- move(?r, ? from, ?to)，表示让机器人从一个位置移动到另一个位置
- pick-up(?r, ?t, ?l)，表示让机器人从某个位置捡起宝藏
- put-down(?r, ?t, ?l)，表示让机器人在某个位置放下宝藏



**第二步，定义复合任务(Compound Task)**

识别出问题中可以被分解的高层任务，在寻宝案例中，最核心的符合任务就是:

- fetch-a-treasure(?r, ?t): 让机器人取回某个宝藏



**第三步，定义方法(Method)**

需要为每一个复合任务编写至少一个方法来告诉规划器怎么分解它

对于fetch-a-treasure, 可以定义一个method-for-fetching的方法



这个方法的前提条件(Precondition)包括:

- 机器人必须在大本营(robot-at ?r base)
- 机器人必须是空闲的(package-empty ?r)
- 宝藏必须在目标地点(treasure-at ?t ?l)



满足了前提条件后，要实现该符合任务可以按照子任务网络(Subtask)的顺序来执行技能:

- 移动到宝藏所在地(move ?r base ?l)
- 拿起宝藏(pick-up ?r ?t ?l)
- 返回大本营(move ?r ?l base)
- 放下宝藏(put-down ?r ?t base)

将上述的3步汇总到domain文件中，

```
(define (domain lost-island-htn)
  ;; HTN有自己的需求标签 :htn
  ...
  (:predicates
    ...
    (treasure-at ?t - treasure ?l - location)
    (package-empty ?r - robot)
    (carrying ?r - robot ?t - treasure)
  )

  ;; =================================================================
  ;;  第一部分：定义原始任务 (Primitive Tasks)
  ;;  这些是机器人可以直接执行的、最基础的动作。
  ;; =================================================================

  (:action move
    :parameters (?r - robot ?from - location ?to - location)
    :precondition (and (robot- ?r ?from))
    :effect (and (robot-at ?r ?to) (not (robot-at ?r ?from)))
  )

  (:action pick-up
    :parameters (?r - robot ?t - treasure ?l - location)
    :precondition (and (robot-at ?r ?l) (treasure-at ?t ?l) (package-empty ?r))
    :effect (and (carrying ?r ?t) (not (package-empty ?r)) (not (treasure-at ?t ?l)))
  )

  ;; 注意：这里的放下动作被严格限制在大本营，直接在底层强制了规则。
  (:action put-down-at-base
    :parameters (?r - robot ?t - treasure)
    :precondition (and (carrying ?r ?t) (robot-at ?r base))
    :effect (and (hand-empty ?r) (treasure-at ?t base) (not (carrying ?r ?t)))
  )

  ;; =================================================================
  ;;  第二部分：定义复合任务与方法 (Compound Tasks & Methods)
  ;;  这是HTN的核心：我们在这里注入“如何做事”的领域知识。
  ;; =================================================================

  ;; (1) 定义一个抽象的、不能直接执行的“复合任务”
  (:task fetch-a-treasure
    :parameters (?r - robot ?t - treasure)
  )

  ;; (2) 定义一个“方法”来告诉规划器如何将上面的复合任务分解
  (:method m-fetch-by-roundtrip
    :parameters (?r - robot ?t - treasure ?l - location)
    
    ;; 这个方法要分解的任务是 fetch-a-treasure
    :task (fetch-a-treasure ?r ?t)
    ...
    ;; 任务分解后的具体步骤（子任务网络）
    :subtasks (and 
        (:ordered   ; 这里的子任务必须按顺序执行
            (move ?r base ?l)
            (pick-up ?r ?t ?l)
            (move ?r ?l base)
            (put-down-at-base ?r ?t)
        )
    )
  )
)
...
初始状态的定义和PDDL一样，描述机器人和宝藏的初始位置

此外，HTN还需要一个初始任务网络，告诉规划器顶层目标是什么

然后就可以得到HTN的问题文件

```
(define (problem find-4-treasures-htn)
  (:domain lost-island-htn)
  ...
  ;; 这里是与PDDL最核心的区别：没有:goal，而是:tasks
  ;; 我们给出了一个顶层的、需要按顺序完成的任务列表
  (:tasks 
    :ordered (
        (fetch-a-treasure r1 coin)
        (fetch-a-treasure r2 goblet)
        (fetch-a-treasure r1 skull)
        (fetch-a-treasure r2 compass)
    )
  )
)
```
...
- https://arxiv.org/pdf/2106.12111
- 2021
- IEEE Transactions on Robotics(TRO)
- **Homepage:** https://brg.engin.umich.edu/publications/robust-task-scheduling/, https://gitlab.com/barton-research-group/open/resilient_team_planner

该论文的核心在于处理不确定性(Uncertainty)，是一个基于随机规划（Stochastic Programming）的\以风险最小化为目标的显式优化算法。



传统的优化目标，是期望最小的成本或者是最大化期望收益的(甚至大部分情况下，算法都没有考虑期望，而是把事件的代价都认为是个常量，那么其期望值也就是常量自身).  但是在一些安全攸关的任务中，这种思路可能是致命的。 比如说，一个计划在平均情况下表现良好，但在5%的最坏情况下彻底失败，如果这是一个灾难救援或者外星探测任务，那么这种风险是不可以接受的。

本文作者认为，一个**稳健(Robust)**的计划，不应该仅仅关心平均情况，而应该管理和优化尾部风险，即那些虽然发生概率不高、但一旦发生后果就非常严重的坏情况。

为此，他们引入了一个金融风险管理中广泛使用的度量指标: **图3:条件风险价值 (Conditional Value at Risk - CVaR)**



![image-20251115200359413](asset/集群任务规划.asset/image-20251115200359413.png)

- **横坐标 CostX** 表示任务完成后，所有可能的成本结果，成本越高，说明结果越差(比如路程更远，能耗更多)
- **纵坐标 Probability desity** 表示每个成本发生的可能性，曲线越高，说明需要这种成本的可能性越高



其中的关键数据:

- **期望值 Expection**(X): 这是所有成本结果的平均值，如果执行无数次任务，那么得到的平均成本就是这个数值。 传统的优化方法一般都是最小化这个"平均成本". 但是这种思路存在一个问题，它忽略了风险，一个计划可能平均成本很低，但是某些情况下，会造成灾难性的高成本。
- **最坏情况 Worst Case sup(X)**: 理论上可能发生的最高的成本。 盯着最坏的情况在做决策也是一种保守的方案，但是这又太悲观了，大部分情况下是杞人忧天，会让计划及其保守和低效。
- **风险容忍度 Probability β**: 一个由决策者(也就是我们自己)设定的百分比数值。 代表我们最关心的那一部分坏结果，比如我们设定 **β=5%** 那么就是那5%最糟糕的可能性，也就是图3中曲线的末尾蓝色的那一部分的面积。 (整个曲线和横坐标围成的面积是100%)
- **条件风险价值 CVaR ηβ(X)**: 在最糟糕的β的可能性中，它们的平均成本是多少? 计算这一部分坏结果的平均值。 CVaR的数值永远会比全局的平均值E(X)要大



有了这个数学工具后，我们就能定量地优化我们的目标: 一个Robust的计划，不是优化平均代价E(X)，而是优化CVaR ηβ(X) - 最小化那5%最坏情况下的平均成本。



本文为此提出了一个框架，APEX-MR

![image-20251115204008593](asset/集群任务规划.asset/image-20251115204008593.png)

1. **第一步：随机建模 (Stochastic Modeling)**
   - **机器人能力**: 将每个机器人的能力（如速度、感知范围等）不再建模为固定数值，而是**随机变量**，遵循某个概率分布（如高斯分布）。
   - **任务需求**: 将完成一个复杂任务所需的能力，同样建模为**随机变量**。
   - **任务结构**: 允许**复杂任务 (Complex Task)** 的存在，即一个任务可以有**多种不同的分解方式**（对应我们之前藏宝任务中的第4个案例）。
2. **第二步：构建一个统一的优化目标**
   - **目标函数**: 构建一个混合目标函数，它需要同时最小化三个部分：
     1. **能源成本 (Energy Cost)**: 机器人完成任务的总能耗。
     2. **时间成本 (Time Cost)**: 完成所有任务的总时间。
     3. **失败风险 (Risk of non-completion)**: 使用**CVaR**来量化“团队总能力**未能满足**任务总需求”这一事件的风险。
   - 通过调整这三项的权重，用户可以在“效率”和“稳健(Robust)”之间做出权衡。
3. **第三步：求解 (A Two-Step Solution Method)**
   - **问题**: 直接求解这个包含随机变量、CVaR和复杂逻辑的“混合整数非线性随机规划”问题，计算上是极其困难的。
   - **解决方案**: 论文提出了一个两步走的解法，来提高可扩展性：
     1. **第一步 (Flow Optimization)**: 首先，不直接规划每个机器人的具体路径。而是求解一个更抽象的“**流网络 (Flow Network)**”问题。它只决定“应该有多少个A类机器人从基地流向任务1，然后再流向任务3...”，而不关心具体是哪个机器人。这大大减少了优化变量的数量。
     2. **第二步 (Flow Decomposition)**: 在得到最优的“机器人流”之后，再进行第二步优化，将这个抽象的“流”分解为**每个独立机器人的具体路径**。
...
​	本文用了一个简单的例子，引出了为什么需要Predictive Optimization，这是我觉得本文写的很好的地方，用循序渐进的方式带领读者理解为什么要Predictive Optimization.



![image-20251114200737120](asset/集群任务规划.asset/image-20251114200737120.png)
...
​	我们可以用之前寻宝的例子套用这个算法，让机器人在一个区域用金属探测仪寻找圣杯。



​	初始化(第一行): 我们设定现在有3个机器人(比之前多1个)，从A/B/C三个初始位置开始探测，形成一个三角形
...
​	进入下一轮循环... A/B/C的位置已经更新为了A'/B'/C'，无人机集群继续飞到新的位置A'/B'/C'，再次执行第3行的感知操作，开始第二轮感知/第二轮决策。



​	在**图2**中，作者针对图1的算法举出了一个潜在的问题: 

​	作者假设了单个机器人的作业场景，也就是在每一次迭代中，A/B/C 3个位置的测量都需要同一个机器人完成，那么在一轮任务中，机器人的先后分配顺序可能是{A1->B1->C1} 或者 {A1->C1->B1}. 

​	如果仅仅看一轮的话，这两个路线所需要的移动距离都是相同的，但是如果我们考虑多次迭代，两者就存在优劣之分: 在{A1->B1->C1}这一轮结束后，机器人要从C1移动到A2，距离记作C1A2;  在{A1->C1->B1}结束后，机器人要从B1移动到A2，距离记作B1A2，从图2可以看出，B1A2的距离明显更短，机器人只需要一半的时间就可以开始下一次的任务。

​	通过这个案例，我们就能理解图1的算法存在短视的问题，所以本文提出了Predictive Optimization的方法。

​	

​	作者还给出了5个Predictive Optimization的其他作用和优势
...
​	![image-20251114205109605](asset/集群任务规划.asset/image-20251114205109605.png)
...
![image-20251114205536450](asset/集群任务规划.asset/image-20251114205536450.png)
...
![image-20251114210126782](asset/集群任务规划.asset/image-20251114210126782.png)
...
- RSS
- 2025.3
- **Homepage**:  https://intelligent-control-lab.github.io/APEX-MR/

![image-20251115144517337](asset/集群任务规划.asset/image-20251115144517337.png)

​	本文专注于**多机械臂协同装配(Cooperative Assembly)**，其算法主要思想包括3个阶段:
​	**任务规划阶段(对应图2.a):** APEX-MR使用 整数线性规划(Integer-Linear Programming, ILP) 来解决初始的任务分配问题(哪个机器人\用哪一个积木\用什么位姿去装配)，得到一个顺序计划(Sequential Plan). 
​	这个过程是一个"回合制"的过程，在任何时刻，只有一个机械臂在工作，其他机械臂都在安全位置(HOME pose)，虽然效率比较低，但是优点是 安全性分析变得非常简单，因为运动的只有一个，所以避碰的运动规划很容易。

​	**运动规划阶段(对应图2.b):** 使用成熟的算法 RRT-Connect 为每个动作生成没有碰撞的路径。

​	**时间计划图阶段(对应图2.c): ** 第一阶段生成的很安全的顺序计划非常低效，因此，本文开始分析
"这个计划中什么等待是没必要的? 哪些动作可以并行?" 
​	它将计划中的每一个动作和路径点，转化为时间计划图(Temporal Plan Graph, TPG)，一个有向无环的图。

- 图中的节点: 表示机械臂在某一个时间点的姿态或者一个完整的动作(比如 抓取)

- 图中的边: 代表了依赖关系，比如A->B，表示B动作要在A动作完成后才可以开始



​	**TPG的构建过程**

- **任务依赖**: 将任务逻辑上的依赖关系（例如，必须先搭好地基才能盖墙）转化为图中的边。
- **碰撞检测**: 然后，算法会检查顺序计划中所有可能并行执行的动作对，看它们是否会发生碰撞。**如果两个动作会碰撞**，就在它们之间加入一条新的边，强制它们一先一后执行。



​	**TPG的执行**

- 在任何时刻，一个机器人可以执行图中的任何一个动作（节点），**只要这个动作的所有“前置依赖”（指向它的边）都已经被满足**。
- **异步并行**: 只要两个机器人的动作之间在TPG中没有依赖路径，它们就可以**完全并行、以各自的速度**去执行，无需相互等待。一个机器人动作快，就先做完；另一个动作慢，也不影响前者。
...
![image-20251114132703360](asset/集群任务规划.asset/image-20251114132703360.png)

![image-20251114150030910](asset/集群任务规划.asset/image-20251114150030910.png)

![image-20251114144556777](asset/集群任务规划.asset/image-20251114144556777.png)
...
## 混合架构 基于显式优化 + 基于学习驱动的决策方法



##### LaMMA-P: A Framework for Language-Model-based Multi-Agent Planning with PDDL

- https://arxiv.org/pdf/2409.20560
- 2025
- ICRA
...
​	在这一小节，我们将探索别的领域中的集群行为，集群的概念其实早在机器人之前就有，只是现在开始被应用到了机器人研究上。

​	一个复杂的系统，必然是从一个一个个体开始构成，达到一定的量级后导致了质变，形成了集群智慧。 我们想要研究好机器人系统，有必要去学习一下别的领域中的现象和算法，同时，也能将我们的算法应用到别的领域。 我们能用机器人的集群算法管理好一群机器人，也能用它作为一个社会治理的系统，管理好整个社会。
